...
public abstract class Expression {
	...
	public void interpret(Context context) {
		...
		if (context.getInput().startsWith(this.nine())) {
			context.setOutput(context.getOutput() + (9 * this.getMultiplier()));
			context.setInput(context.getInput().substring(2));
		} else if (context.getInput().startsWith(this.four())) {
			context.setOutput(context.getOutput() + (4 * this.getMultiplier()));
			context.setInput(context.getInput().substring(2));
		} else if (context.getInput().startsWith(this.five())) {
			context.setOutput(context.getOutput() + (5 * this.getMultiplier()));
			context.setInput(context.getInput().substring(1));
		}

		while (context.getInput().startsWith(this.one())) {
			context.setOutput(context.getOutput() + (1 * this.getMultiplier()));
			context.setInput(context.getInput().substring(1));
		}
	}
